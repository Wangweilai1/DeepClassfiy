...
loss=tf.losses.sparse_softmax_cross_entropy(labels=y_,logits=finaloutput)
train_op=tf.train.AdamOptimizer(learning_rate=0.0001).minimize(loss)
#train_op=tf.train.GradientDescentOptimizer(0.5).minimize(loss)

correct_prediction = tf.equal(tf.cast(tf.argmax(finaloutput,1),tf.int32), y_)    
