...
def build_network(height, width, channel):
    ...
    y = tf.placeholder(tf.int32, shape=[None, 2], name='labels_placeholder')
    ...
    optimize = tf.train.AdamOptimizer(learning_rate=1e-2).minimize(cost)
    #optimize = tf.train.GradientDescentOptimizer(0.5).minimize(cost)

    prediction_labels = tf.argmax(finaloutput, axis=1, name="output")
    read_labels = y

    correct_prediction = tf.equal(tf.cast(prediction_labels, tf.int32), read_labels)
    ...
    with tf.Session() as sess:
        ...
        for epoch_index in range(num_epochs):
            for i in range(len(x_train)):
                ...
                mean_cost_in_batch, prediction, output_fc8 = sess.run([graph['cost'],graph['finaloutput'], graph['output_fc8']], feed_dict={
                        graph['x']: np.reshape(x_train[i], (1, 224, 224, 3)),
                        graph['y']: ([[1, 0]] if y_train[i] == 0 else [[0, 1]])
                    })
                print('Epoch - {:2d}, loss_on_train:{:6.6f}, pred: {}, fc8: {}, True: {}'.format(epoch_index, mean_cost_in_batch, prediction, output_fc8, [[1, 0]] if y_train[i] == 0 else [[0, 1]]))
            if epoch_index % epoch_delta == 0:
                ...
                print('Epoch - {:2d}, acy_on_test:{:6.6f}%({}/{}),loss_on_test:{:6.6f}, acy_on_train:{:6.6f}%({}/{}),loss_on_train:{:6.6f}'.format(epoch_index, acy_on_test*100.0,total_correct_times_in_test_set,
